...
> Codex slash-prompt. Originally derived from `.claude/skills/create-stories/SKILL.md` (Claude-Code template fork — see `docs/codex/README.md`).
...
in this skill — apply the check pattern from `docs/codex/refs/director-gates.md`
...
After decomposing all stories (Step 4 complete) but before presenting them for write approval, spawn `qa-lead` via the relevant /agent-<name> Codex prompt using gate **QL-STORY-READY** (`docs/codex/refs/director-gates.md`).
...
Use an inline question to the user to close with context-aware next steps:
