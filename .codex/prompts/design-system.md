...
2. If it exists: read it, find the highest-priority system with status "Not Started" or equivalent, and use an inline question to the user:
...
- Read `docs/codex/refs/technical-preferences.md` to identify the engine and version
...
Use the template structure from `docs/codex/refs/templates/game-design-document.md`:
...
2. **Questions**: Ask clarifying questions specific to this section. Use
   an inline question to the user for constrained questions, conversational text for open-ended
   exploration.
...
3. **Options**: Where the section involves design choices (not just documentation),
   ...
   then use an inline question to the user to capture the decision.
...
6. **Approval**: Immediately after the draft — in the SAME response — use
   an inline question to the user. **NEVER use plain text. NEVER skip this step.**
...
**Framing questions (ask BEFORE drafting)**: Use an inline question to the user with a multi-tab widget:
...
**Framing question (ask BEFORE drafting)**: Use an inline question to the user:
...
spawn `creative-director` via the relevant /agent-<name> Codex prompt:
...
**Agent delegation (MANDATORY)**: Before drafting Section C, spawn specialist agents via the relevant /agent-<name> Codex prompt in parallel:
...
- Surface any disagreements between agents to the user via an inline question to the user
...
**Agent delegation (MANDATORY)**: Before proposing any formulas or balance values, spawn specialist agents via the relevant /agent-<name> Codex prompt in parallel:
...
- Present the specialists' proposals to the user for review via an inline question to the user
...
For required systems: **spawn `art-director` via the relevant /agent-<name> Codex prompt** before drafting this section. Provide: system name, game concept, game pillars, art bible sections 1–4 if they exist. Ask them to specify: (1) VFX and visual feedback requirements for this system's events, (2) any animation or visual style constraints, (3) which art bible principles most directly apply to this system. Present their output; do NOT leave this section as `[To be designed]` for visual systems.
...
Before finalizing the GDD, spawn `creative-director` via the relevant /agent-<name> Codex prompt using gate **CD-GDD-ALIGN** (`docs/codex/refs/director-gates.md`).
...
**When delegating via /agent-<name> prompt**:
...
- The main session presents the agent's output to the user via an inline question to the user
- The user decides; the main session writes to file
