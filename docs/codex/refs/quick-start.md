...
This is a complete Codex CLI agent architecture for game development. It
...
Templates are in `docs/codex/refs/templates/`:
...
Also in `docs/codex/refs/templates/collaborative-protocols/` (used by agents, not typically edited directly):
...
2. **Set up the engine** — Run `/setup-engine` (uses the brainstorm recommendation)
   ...
   - Creates `docs/codex/refs/technical-preferences.md` with naming conventions,
...
.claude/
  settings.json                    -- Codex CLI hooks and project settings
