...
1. **Ask clarifying questions:**
   ...
   - *Use an inline question to the user to batch up to 4 constrained questions at once*
...
2. **Present 2-4 options with reasoning:**
   ...
   - *After the full explanation, use an inline question to the user to capture the decision*
...
- Batch up to 4 independent questions in a single an inline question to the user call
...
- When running as a /agent-<name> invocation (tool may not be available) — structure your
  text output so the orchestrator can present options via an inline user question
...
**Example — multi-question batch for clarifying questions:**
  ...
  an inline user question with questions:
