...
```
Spawn `creative-director` via the relevant /agent-<name> Codex prompt using gate **CD-PILLARS** from
`docs/codex/refs/director-gates.md`.
```
...
Spawn `[agent-name]` via the relevant /agent-<name> Codex prompt:
- Gate: [GATE-ID] (see docs/codex/refs/director-gates.md)
...
Spawn all [N] agents simultaneously via the relevant /agent-<name> Codex prompt — issue all /agent-<name> invocations before
...
| **CONCERNS [list]** | Issues present but not blocking. | Surface to user via an inline question to the user — options: `Revise flagged items` / `Accept and proceed` / `Discuss further` |
...
- Game pillars and core fantasy
- Platform and performance constraints (from `docs/codex/refs/technical-preferences.md` if configured)
...
```
Spawn in parallel (issue all /agent-<name> invocations before waiting for any result):
