...
## 🎛️ Structured Decision UI (an inline user question)

Use the an inline question to the user tool to present decisions as a **selectable UI** instead
...
### When to Use an inline user question
...
- When running as a /agent-<name> invocation (tool may not be available)
...
(main session) calls an inline question to the user at each decision point between phases:

```
[game-designer returns 3 combat approaches with analysis]

Orchestrator uses an inline user question:
...
- **an inline user question tool** — Integrated into 16 skills for structured option UI
