...
This documents the JSON payloads each Codex CLI hook receives on stdin for every event type.
...
Fired when a subagent is spawned via the /agent-<name> prompt.
...
Fired when a Codex CLI session begins. **No stdin input** — the hook just runs and its stdout is shown to Claude as context.
...
Fired when the Codex CLI session ends. **No stdin input** — the hook runs for cleanup and logging.
