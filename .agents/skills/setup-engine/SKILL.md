...
**Question 1 — Prior experience** (ask this first, always, via an inline question to the user):
...
**Question 2 — Target platform** (ask this second, always, via an inline question to the user — platform eliminates or heavily weights engines before any other factor):
...
5. Use an inline question to the user to confirm: "Does this recommendation feel right, or would you like to explore a different engine?"
...
Use an inline question to the user with concept-specific deep-dive topics. Always generate these options from the user's actual concept — do not use generic options. Always include at minimum:
...
After updating AGENTS.md, create or update `docs/codex/refs/technical-preferences.md` with
...
### Remaining Sections
- **Performance Budgets**: Use an inline question to the user:
