...
3. **Run through ideation phases** interactively, asking the user questions at
   ...
   **Use an inline question to the user** at key decision points throughout brainstorming:
   ...
   an inline question to the user to capture the decision with concise labels.
...
- What 3 games have you spent the most time with? What kept you coming back?
  *(Ask this as plain text — the user must be able to type specific game names freely.
  Do NOT put this in an an inline user question with preset options.)*
- Are there genres you love? Genres you avoid? Why?
- Do you prefer games that challenge you, relax you, tell you stories,
  or let you express yourself? *(Use an inline question to the user for this — constrained choice.)*

**Practical constraints** (shape the sandbox before brainstorming).
Bundle these into a single multi-tab an inline question to the user with these exact tab labels:
...
Present all three. Then use an inline question to the user to capture the selection.

**CRITICAL**: This MUST be a plain list call — no tabs, no form fields. Use exactly this structure:

```
an inline user question(
...
Ask these as an inline question to the user calls — derive the options from the chosen concept, don't hardcode them:
...
**Pillar confirmation**: After presenting the full pillar set, use an inline question to the user:
...
If the user selects B, C, or D, make the revision, then use an inline question to the user again:
...
**After pillars and anti-pillars are agreed, spawn BOTH `creative-director` AND `art-director` via the relevant /agent-<name> Codex prompt in parallel before moving to Phase 5. Issue both /agent-<name> invocations simultaneously — do not wait for one before starting the other.**

- **`creative-director`** — gate **CD-PILLARS** (`docs/codex/refs/director-gates.md`)
  Pass: full pillar set with design tests, anti-pillars, core fantasy, unique hook.

- **`art-director`** — gate **AD-CONCEPT-VISUAL** (`docs/codex/refs/director-gates.md`)
...
Collect both verdicts, then present them together using a two-tab an inline question to the user:
...
- **Target platform**: Use an inline question to the user — "What platforms are you targeting for this game?"
...
- **Engine experience**: Use an inline question to the user — "Do you already have an engine you work in?"
...
**After identifying biggest technical risks, spawn `technical-director` via the relevant /agent-<name> Codex prompt using gate TD-FEASIBILITY (`docs/codex/refs/director-gates.md`) before scope tiers are defined.**
...
**After scope tiers are defined, spawn `producer` via the relevant /agent-<name> Codex prompt using gate PR-SCOPE (`docs/codex/refs/director-gates.md`).**
...
4. **Generate the game concept document** using the template at
   `docs/codex/refs/templates/game-concept.md`. Fill in ALL sections from the
...
5. Use an inline question to the user for write approval:
...
If [B]: ask which section to revise using an inline question to the user with options: `Elevator Pitch` / `Core Fantasy & Unique Hook` / `Pillars` / `Core Loop` / `MVP Definition` / `Scope Tiers` / `Risks` / `Something else — I'll describe`

After revising, show the updated section as a diff or clear before/after, then use an inline question to the user — "Ready to write the updated concept document?"
Options: `[A] Yes — write it` / `[B] Revise another section`
Repeat until the user selects [A].

If yes, generate the document using the template at `docs/codex/refs/templates/game-concept.md`, fill in ALL sections from the brainstorm conversation, and write the file, creating directories as needed.
