...
Before generating the Go/No-Go recommendation, spawn `producer` via the relevant /agent-<name> Codex prompt using gate **PR-MILESTONE** (`docs/codex/refs/director-gates.md`).
...
If OFF TRACK, use an inline question to the user before generating the recommendation:
...
If AT RISK, use an inline question to the user:
