...
a **confirm/adjust** prompt using an inline question to the user — not open-ended questions.
...
decisions remain, present each one as a separate an inline question to the user with the proposed
...
5.5. **Engine Specialist Validation** — Before saving, spawn the **primary engine specialist** via the relevant /agent-<name> Codex prompt to validate the drafted ADR:
   - Read `docs/codex/refs/technical-preferences.md` `Engine Specialists` section to get the primary specialist
...
5.6. **Technical Director Strategic Review** — After the engine specialist validation, spawn `technical-director` via the relevant /agent-<name> Codex prompt using gate **TD-ADR** (`docs/codex/refs/director-gates.md`):
...
5. **Write approval** — Use an inline question to the user:
...
Ask using an inline question to the user:
...
After the ADR is written (and registry optionally updated), close with an inline question to the user.
